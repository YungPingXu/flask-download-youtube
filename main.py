...
def download_thread(title, url):
	process = Popen('youtube-dl -o \"' + MEDIA_ROOT + title + '.mp4\" -f bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4 ' + url, stdout=PIPE, stderr=STDOUT, shell=True, universal_newlines=True, encoding="utf-8")
	output = ''
	while True:
		line = process.stdout.readline()
		if not line:
...
def mp3cut_thread(title, start, end):
	process = Popen('ffmpeg -y -i \"' + MEDIA_ROOT + title + '.mp4\" -ss ' + start + ' -to ' + end + ' \"' + MEDIA_ROOT + title + '_mp3cut_' + start + '_' + end + '.mp3\"', stdout=PIPE, stderr=STDOUT, shell=True, universal_newlines=True, encoding="utf-8")
	output = ''
	while True:
		line = process.stdout.readline()
		if not line:
...
def mp4cut_thread(title, start, end):
	process = Popen('ffmpeg -y -i \"' + MEDIA_ROOT + title + '.mp4\" -ss ' + start + ' -to ' + end + ' -acodec copy \"' + MEDIA_ROOT + title + '_mp4cut_' + start + '_' + end + '.mp4\"', stdout=PIPE, stderr=STDOUT, shell=True, universal_newlines=True, encoding="utf-8")
	output = ''
	while True:
		line = process.stdout.readline()
		if not line:
